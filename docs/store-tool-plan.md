...
Status: revision 2, approved 2026-08-19; Phases 1-8 complete (Braele iPad/locales pending captures and copy; Play feature graphic + Play text metadata open)
...
- [x] Post-v1 — release review + editor ergonomics (2026-08-21: Release tab with generated-PNG gallery per locale/target, staleness via the renderer's inputs hash, metadata summary and per-locale sign-off persisted in `store/release-signoff.json`; per-family device shell (`shell` accepts `{ iphone, ipad, ... }`, editor dropdown edits the selected target's slot); Strip mode edits the selected frame like Single mode; canvas click/double-click fixed under pointer capture; screens-list double-click opens Single mode; undo/redo verified end to end)
- [x] v1.0.0 — first tagged release of the tool (2026-08-21)
