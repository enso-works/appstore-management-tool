...
| `source.missing`                                       | Raw capture not found at `store/raw/<device>/<locale>/<file>`; capture it (`store-shots capture --screen <id> --device <iphone                                                      | ipad> --locale <l>`) or fix `source.filePattern` |
